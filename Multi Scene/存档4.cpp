...
int main( int argc, char** argv )
{
	...
	char directory1[MAX_PATH] = "D:\\VIPeR\\img1";
	result1 = EnumFiles(directory1, &count1);
	char directory2[MAX_PATH] = "D:\\VIPeR\\img2";
 ...
 //储存基准图像和另外两张对比图像的矩阵( RGB 和 HSV )
	...
	/// 装载三张背景环境不同的图
	for (i = 0; i < 10; ++i) {
		path1 = "D:\\VIPeR\\img1\\" + (string)result1[i+2];
		src_base = imread(path1);
		for (j = 0; j < count2 - 3; ++j) {
			path2 = "D:\\VIPeR\\img2\\" + string(result2[j+2]);
			...
			double base_half = compareHist( histbase_half_down, hist_half_down, CV_COMP_BHATTACHARYYA);
			double base_half2 = compareHist( histbase_half_up, hist_half_up, CV_COMP_BHATTACHARYYA);
			half_relativity[j] = base_half+base_half2;
			half_ordered_relativity[j] = base_half+base_half2;
		}
		qsort(half_ordered_relativity, count2 - 3, sizeof(double), cmp);
		cout << "图片" << result1[i+2] << "对应的是" << endl;
		for (int t = 0; t < 27; ++t) {
			for (int k = 0; k < count2 - 3; ++k) {
