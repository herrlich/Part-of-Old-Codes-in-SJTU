 ...
 //储存基准图像和另外两张对比图像的矩阵( RGB 和 HSV )
	...
	for (i = 0; i < 25; ++i) {
		...
		for (j = 0; j < 5; ++j) {
			...
			double base_half = compareHist( hist_base, hist_half_down, CV_COMP_CORREL);
			relativity[j] = base_test1;
			ordered_relativity[j] = base_test1;
			half_relativity[j] = base_half;
			half_ordered_relativity[j] = base_half;
		}
		...
		for (int t = 0; t < 5; ++t) {
			for (int k = 0; k < count2 - 2; ++k) {
				...
				if (fabs(half_relativity[k] - half_ordered_relativity[t]) < 1E-10) cout << result2[k+2] << endl;
			}
		}
	}
	delete [] relativity;
