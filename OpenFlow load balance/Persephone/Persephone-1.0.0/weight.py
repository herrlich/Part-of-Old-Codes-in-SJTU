...
def array():
	...
	for swit in range(10000):
		...
		for ctrl in range(100):
			...
			if (dist <= 30 * 30 and dist > 20 * 20):
				potential_matrix[swit][ctrl] = 1
			if (dist <= 20 * 20 and dist > 10 * 10):
				potential_matrix[swit][ctrl] = 2
			if (dist <= 10 * 10):
				potential_matrix[swit][ctrl] = 3
			if (dist > 30 * 30):
	...
	for i in range(10000):
		...
		if (num_pot_ctrl[current_seq] == 1):
			for j in range(100):
				if (potential_matrix[current_seq][j] > 0):
			...
			for k in range(100):
				if (potential_matrix[current_seq][k] > 0):
			...
			controller_list[tmp_ctrl] += switch_list[current_seq]
	...
	for time in range(1):
		...
		for i in range(100):
			for j in range(len(real_controller[i])):
				total_value += potential_matrix[real_controller[i][j]][i]
		print (total_value)
		...
		while (len(overlist) != 0):
			...
			for i in range(100):
				...
				if (adjacent_controller[cnum][j] == 1 and controller_list[j] < thd[j]):
					#print j
					for m in range(len(real_controller[cnum])):
						for n in range(m+1, len(real_controller[cnum])):
							if (potential_matrix[real_controller[cnum][n]][j] > potential_matrix[real_controller[cnum][m]][j]):
								tmp_value = real_controller[cnum][m]
								real_controller[cnum][m] = real_controller[cnum][n]
								real_controller[cnum][n] = tmp_value

					for q in range(4):
						for s in range(len(real_controller[cnum])):
							for t in range(s+1, len(real_controller[cnum])):
								#print i + s + t
								if (potential_matrix[real_controller[cnum][t]][j] == (3 - q) and potential_matrix[real_controller[cnum][s]][j] == (3 - q) and switch_list[real_controller[cnum][t]] > switch_list[real_controller[cnum][s]]):
					...
					for k in range(len(tmp_controller)):
						snum = tmp_controller[k]
						if (potential_matrix[snum][j] > 0 and controller_list[j] < thd[j]):
		...
		for i in range(100):
			for j in range(len(pendlist)):
				if (adjacent_controller[i][pendlist[j]] == 1):
					if (i not in full_pend_list):
						full_pend_list.append(i)
						break

		for i in range(len(pendlist)):
			if (i not in pendlist):
				full_pend_list.append(pendlist[i])
		...
		if (len(pendlist) != 0):
			...
			for i in range(len(full_pend_list)):
				total_pend += controller_list[full_pend_list[i]]
				total_capacity += controller_max[full_pend_list[i]]
			pend_percentage = total_pend / total_capacity

		for i in range(len(pendlist)):
			while (controller_list[pendlist[i]] > pend_percentage * controller_max[pendlist[i]] * gamma):
				cmin = 10000
				cnum = 0
				for j in range(len(full_pend_list)):
				...
				for m in range(len(real_controller[pendlist[i]])):
					for n in range(m+1, len(real_controller[pendlist[i]])):
						if (potential_matrix[real_controller[pendlist[i]][n]] > potential_matrix[real_controller[pendlist[i]][m]]):
							tmp_value = real_controller[pendlist[i]][m]
							real_controller[pendlist[i]][m] = real_controller[pendlist[i]][n]
							real_controller[pendlist[i]][n] = tmp_value

				for q in range(4):
					for m in range(len(real_controller[pendlist[i]])):
						for n in range(m+1, len(real_controller[pendlist[i]])):
							if (real_controller[pendlist[i]][m] == (3 - q) and real_controller[pendlist[i]][n] == (3 - q) and switch_list[real_controller[pendlist[i]][m]] < switch_list[real_controller[pendlist[i]][n]]):
				...
				controller_list[cnum] += switch_list[real_controller[pendlist[i]][0]]
				real_controller[pendlist[i]].remove(real_controller[pendlist[i]][0])
				controller_list[pendlist[i]] = controller_list[pendlist[i]] - switch_list[real_controller[pendlist[i]][0]]
				ch_swit_num += 1
