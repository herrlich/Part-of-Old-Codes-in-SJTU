...
def array():
	...
	for time in range(10):
		...
		for i in range(100):
			aver_ctrl_last[i] = aver_ctrl_now[i]
		...
		outcome = 0
		for i in range(100):
			outcome += (controller_list[i] - aver_last) * (controller_list[i] - aver_last)
		outcome = outcome / 100
		print(outcome)

		
		# calculate the threshold and efn for each controller
		thd = []
		efn = []
		for i in range(100):
			thd_value = aver_ctrl_now[i] * alpha + aver_ctrl_last[i] * (1 - alpha) 
			efn_value = beta * thd_value
			thd.append(thd_value)
			efn.append(efn_value)

		# for each controller, do the migration part without thinking about its rank
	
		# shuffle the controller array
		controller_array = []
		for i in range(100):
			controller_array.append(i)
		random.shuffle(controller_array)

		for i in range(100):
			cnum = controller_array[i]
			adj_ctrl_num = 1

			for m in range(len(real_controller[cnum])):
				for n in range(m+1, len(real_controller[cnum])):
					if (switch_list[real_controller[cnum][n]] > switch_list[real_controller[cnum][m]]):
						tmp_switch = real_controller[cnum][m]
						real_controller[cnum][m] = real_controller[cnum][n]
						real_controller[cnum][n] = tmp_switch

			tmp_controller = []
			for k in range(len(real_controller[cnum])):
				tmp_controller.append(real_controller[cnum][k])
		
			if (controller_list[cnum] > efn[cnum]):
				for j in range(len(tmp_controller)):
					snum = tmp_controller[j]
					tmp_min = 10000
					tmp_min_ctrl = 0
					for k in range(100):
						if (adjacent_controller[cnum][k] == 1 and controller_list[k] < efn[k] and controller_list[k] < tmp_min):
							tmp_min = controller_list[k]
							tmp_min_ctrl = k
					real_controller[tmp_min_ctrl].append(snum)
					controller_list[tmp_min_ctrl] += switch_list[snum]
