...
bool end_up=true;//end_up用来判断是否过滤了一次以后剩下的字符串中仍有敏感字符：如字符串assddf，敏感字符为sd，第一次过滤后剩下asdf，仍有敏感字符
...
int main(){
	...
	}while(!end_up);//此处循环为关键，当和谐一次以后的字符串中仍有敏感字符时，end_up为false，继续循环，直至敏感字符全部除去为止
...
void judge_undefined(char string[],char punctuation[],char useless[]){
	int i,j;
	real_length=strlen(string);
	//因为在每次循环去除敏感字符时，都是从一个新的string字符串开始（即第一次从原字符串中去除了一次敏感字符后的新字符串覆盖原字符串，下一次检测的是新字符串），所以需要每次都重新测量以下字符串的长度
	for (i=0;i<500;++i) type[i]=0;//需要重新使type所有的元素的值都为0，因为随数组更新，type数组的值也要重新来赋
	for (i=0;i<real_length;++i){
		...
		if (type[i]==0){	//此处判断是否为0是为了避免已有编号的字符进入下一循环浪费时间
...
void transform(int type[],char string[]){
	int i,total=0;
	count=0;//因为每次新开始一轮检测时都是用了一个上一轮和谐后的字符串，所以再次测量化简后字符串个数时必须清零再开始
	for (i=0;i<real_length;++i){
...
//作用：此函数用来和谐要和谐的敏感字符串（但若要和谐复合的敏感字符串时，如ssdd，则要将此函数执行若干次）
void eliminate(char *sensitive[],int sensitive_length[],char string[]){
	end_up=true;//每次开始时将判断和谐穷尽的变量赋值为true，如果没有字符可和谐时，就可以在do/while循环中退出
	int i,j,*final_judge;
	...
	for (i=0;i<sensitive_num;++i){   //从每一个敏感词汇开始
		for (j=0;j<=count-sensitive_length[i];++j){    //到首字符能到的最远距离
			if (strncmp(simplified_str+j,sensitive[i],sensitive_length[i])==0&&final_judge[j]==0){
				...
				end_up=false;//只要在本轮中有一次和谐，判断是否和谐穷尽的变量就被赋值为false
				final_judge[j]=1;
				final_judge[j+sensitive_length[i]-1]=1;
				//只标记开头和结尾
			}
		}
	}
	...
	for (i=0;i<=temp;++i) string[i]=ultimate_string[i];
	//用ultimate_string的值来更新string的值，即下一轮只针对上一轮被和谐过的string来和谐，而不用去管前一轮的字符串是什么样子的
	delete [] final_judge;//回收本轮的动态空间，因为本次和谐过以后可能还有下一轮和谐，所以要将这些每轮都要用到的动态数组回收，以便下轮重建
	delete [] mark;
