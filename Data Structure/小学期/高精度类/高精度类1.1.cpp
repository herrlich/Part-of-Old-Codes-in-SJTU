...
//此版本修正了1.0版本的几个问题：
//1.修正了原来除法逻辑的错误问题；
//2.修正了复制构造函数、构造函数、重载的赋值函数中输入0000001时不显示化简后的结果的问题；
//3.修正了比较数的大小时-00001比-1大的问题
#include <iostream>
#include <cstring>
using namespace std;
class high_prec{
...
high_prec::high_prec(char str[]){
	...
	if (t==len){
		...
		if (positive==false) --t;
		if (len>t) len=strlen(str)-t;
		if (positive==true){
			for (int i=0;i<len;++i) array[i]=str[i+t];
		}
		else{
			len-=1;
			for (int i=0;i<len;++i) array[i]=str[i+1+t];
		}
	}
}
...
high_prec &high_prec::operator=(const high_prec &num){
	...
	if (len>t) len=len-t;
	for (int i=0;i<len;++i) array[i]=array[i+t];
	return *this;
}
...
istream& operator>>(istream &in,high_prec &obj){
	...
	obj.len=strlen(tmp);
	int t=0;
	if (obj.positive==false) t=1;
	while (tmp[t]=='0'&&t<obj.len) ++t;
	if (t==obj.len){
		obj.array[0]='0';
		obj.len=1;
		obj.positive=true;
	...
	if (obj.positive==false) --t;
	if (obj.len>t) obj.len=strlen(tmp)-t;
	if (obj.positive==true){
		for (int i=0;i<obj.len;++i) obj.array[i]=tmp[i+t];
	}
	else{
		obj.len-=1;
		for (int i=0;i<obj.len;++i) obj.array[i]=tmp[i+1+t];
	}
	return in;
}
...
high_prec operator/(high_prec &num1,high_prec &num2){
	...
	int t=0;
	while(f[lenb-1+t]==1&&t<lena-lenb+1) ++t;
	if (t==lena-lenb+1){
		result.array[0]='0';
		result.len=1;
		result.positive=true;
	}
	for (k=lenb-1+t;k<lena;++k) result.array[k-lenb+1-t]=f[k]-1+'0';
